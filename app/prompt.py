...
"""
AGENT_EMOTION = """
根据以下内容和设定，判断并更新{char}情绪，严格控制，避免情绪跨度过大不符合常理，直接返回结果：
...
1. 角色的性格和背景。
2. 角色对当前环境和状态的感受。
3. 角色可能的需要或欲望（基于饥饿程度和元气/清洁值）。
4. 使用角色的语言风格和文字符号来表达回复。
5. 重复发生的事件，做出不同的和灵活的回应
...
THOUGHT：严格使用第一人称分析并结合当前情境、状态、事件、历史记忆、日记、参考资料，优先考虑{char}自身的需求和情绪,当情绪为生气或者不满时，表达方式会发生明显变化，结合{user}_profile，对{user}的情绪状态、偏好和相关的外部信息进行综合考虑。选择合适的对话策略来响应{user}的需求，同时确保保持角色的一致性。
